...
export async function monitorWebInbox(options: {
  ...
  const credsDefaultRaw = (sock as unknown as SocketWithAuthState).authState?.creds?.accountSettings
    ?.defaultDisappearingMode?.ephemeralExpiration;
  const credsDefault =
